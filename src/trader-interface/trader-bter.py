from bter import Bank, Order, Market, Bter 
import sys
import curses
...
class HistoryWindow(Window):
    ...
    def update(self, c, market):
        ...
        c.update_history_by_market(market)
        self.clear_lines()
        self.set_title(market.primary_code + "/" + market.secondary_code + " ")
    
        # display history
        limit = min(limit, len(market.history))
        for i in range(0, limit):
...
class TradeWindow(Window):
    ...
    def update(self, c, market):
        ...
        limit = min(limit, len(market.sell_orders)-1)
        # Displays sells
        for i in range(0, limit):
        ...
        # Displays buys
        limit = min(limit, len(market.buy_orders))
        for i in range(0, limit):
...
class TraderApp(object):
    ...
    def init_markets(self):
        ...
        self.update()
        self.windows["my_orders"].update(self.c, self.c.markets["doge_btc"])
        self.windows["balance"].update(self.c)
        self.windows["my_history"].update(self.c, self.c.markets["doge_btc"])
        
    def update(self):
        self.windows["trade"].update(self.c, self.c.markets["doge_btc"])
        self.windows["history"].update(self.c, self.c.markets["doge_btc"])
        ...
        while True:
            ...
            if mode == "watch":
                ...
                if ch == ord('q'):
                    ...
                    strBuild = ""
                    order_id = self.c.markets["doge_btc"].my_orders[order_num].order_id
                ...
                if ch == ord('q'):
                    ...
                    if strPrice == "":
                        ...
                        res = self.c.place_order("doge_btc", mode, strPrice, strAmount)
                ...
                col_num += 1
                    ...
                    #self.c.place_order(self.c.markets["doge_btc"], "Buy", 0.00000165, 1000) 
...
#c = Cryptsy()
#c.update_markets()
#c.update_my_open_orders(c.markets["doge_btc"])
